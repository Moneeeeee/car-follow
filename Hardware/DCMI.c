...
void DCMI_IRQHandler(void)
{
	if(DCMI_GetITStatus(DCMI_IT_FRAME)==SET)//捕获到一帧图像
	{
       ...
       // 获取当前 SysTick 值
        ...
        // 计算距离上一次帧中断的时间（以毫秒为单位）
        uint32_t elapsed_time = (last_tick - current_tick) / SystemCoreClock * 1000;

        // 计算帧率
        uint32_t frame_rate = 1000 / elapsed_time;

        // 更新上一次测量的 SysTick 值
        last_tick = current_tick;
        LCD_ShowNum(200,200,frame_rate,100,10);
        // 现在，帧率存储在 frame_rate 变量中
	}
} 
//DCMI DMA配置
