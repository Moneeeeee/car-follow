...
void Show_Picture(u16 x,u16 y,u16 height,u16 width,u16 *p)
{
	u8 i,j;
/****************************************/
/*	                                    */ 
...
//				buf[i*2+1][j*2]=p[i][j];
//				buf[i*2][j*2+1]=p[i][j];
//				buf[i*2+1][j*2+1]=p[i][j];
//			}
//		}
	for(i=0;i<height/2;i++)
		{
   
				LCD_SetCursor(x,y+2*i);
				LCD_WriteRAM_Prepare();
				for(j=0;j<width/2;j++)
				{
					LCD->LCD_RAM=p[(i-1)*width/2+j];
					LCD->LCD_RAM=p[(i-1)*width/2+j];
				}
				LCD_SetCursor(x,y+2*i+1);
				LCD_WriteRAM_Prepare();
				for(j=0;j<width/2;j++)
				{
					LCD->LCD_RAM=p[i*width/2+j];
					LCD->LCD_RAM=p[i*width/2+j];
				}
		}
}
...
//void Show_GrayPicture(u16 x,u16 y,u16 height,u16 width,u16 *p)
//{
//	u16 i,j,z;
//	u8 red,green,blue;
//	u8 gray8[240][400];
...
//			gray16=gray8[i][j]>>3;
//			color[z]=(0x001f&gray16)<<11;
//			color[z]=color[z]|(((0x003f)&(gray16*2))<<5);
//			color[z]=color[z]|(0x001f&gray16);
//			z++;
//		}
//	}
//	for(i=0;i<height/2;i++)
...
//				for(j=0;j<width/2;j++)
//				{
//					LCD->LCD_RAM=color[(i-1)*width/2+j];
//					LCD->LCD_RAM=color[(i-1)*width/2+j];
//				}
//				LCD_SetCursor(x,y+2*i+1);
//				LCD_WriteRAM_Prepare();
